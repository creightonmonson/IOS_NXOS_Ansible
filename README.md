...
In order to maximize modularity and readability, each play or playbook will reference a role of a same or similar name in the roles directory. Also, variables and loops are maximized to increase scalability. Below is the general structure:
```
General Use Playbooks:

- backup_config.yml    --> generates a text file of device config 
- get facts.yml        --> displays device info (model, s/n, interfaces config)

New Device Onboarding:

- new_device.yml       --> imports all of the below playbooks to consolidate the onboarding of a new device
- configure_banner.yml --> configure login banner
- configure_dns.yml    --> configures domain and dns servers
- configure_ntp.yml    --> configures ntp server, vrf, and source interface
- configure_snmp.yml   --> configures snmp server
- mgmt_static_route.yml--> configures static routes for management vrf to reach network resources (ie ntp, dns, snmp)

Network Deploy:

- network_deploy.yml       --> configures all network devices to meet desired end state. Imports playbooks for IOS device (below) and uses roles for NXOS devices
- ios_l3_config.yml        --> generates interface config using jinja template (saved in configs directory) and then deploys to network device 
- ios_ospf_config.yml      --> generates OSPF config using jinja template (saved in configs directory) and then deploys to network device
- access_vlans_config.yml  --> generates vlan config using jinja template (saved in configs directory) and then deploys to network device
- access_uplinks_config.yml--> generates port-channel config using jinja template (saved in configs directory) and then deploys to network device
```
Key changes from main branch are using jinja templates for all IOS configurations, specifically the playbooks:
```
- ios_l3_config.yml
- ios_ospf_config.yml
...
## Future Developments
Convert all roles to jinja templates, which reduces run time and dependency on ansible-galaxy modules. Also add syslog configure playbook for new_device